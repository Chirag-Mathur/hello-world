...
int cal(int n,int ar,int as,int ap,int br,int bs,int bp)
{
    int ans=0;
    if(ar){
        if(bs>=ar){
...
void fun(){
    int n,ar,as,ap,br,bs,bp;cin>>n>>ar>>as>>ap>>br>>bs>>bp;
    int smallass=((ar-bp>0)?ar-bp:0)+((as-br>0)?as-br:0)+((ap-bs>0)?ap-bs:0);
    int bigass=cal(n,ar,as,ap,br,bs,bp);
    cout<<smallass<<" "<<bigass;
}
